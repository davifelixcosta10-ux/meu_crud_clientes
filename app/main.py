...
app = FastAPI(title="DaviFlow API v1.2")
...
# Helper para extrair o user_id passado via Header Authorization
...
def login(dados: UserLogin):
    try:
        ...
        # Correção: acessa res.session.user e corrige 'access_token'
        return {
...
# Correção no nome da rota: {cliente_id} no singular
...
def atualizar_cliente(cliente_id: str | int, dados: ClienteUpdate, authorization: str = Header(None)):
    ...
    try:
        ...
        if not campos:
            # Se nada foi alterado, busca os dados atuais
